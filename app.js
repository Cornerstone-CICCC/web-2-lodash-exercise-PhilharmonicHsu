...
const californiaCities = _.chain(cities)
  .filter(cityGroup => cityGroup.state === 'California')
...
const texasCities = _.chain(cities)
  .filter(cityGroup =>
    cityGroup.state === 'Texas' &&
    cityGroup.population < 1000000
  )
  .map(city => city.cityName)
  .value()
...
const californiaPopulation = _.chain(cities)
  .filter(cityGroup => cityGroup.state === 'California')
  .reduce((sum, n) => sum + n.population, 0)
  .value()
